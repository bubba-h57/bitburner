...
export class Mobster {
        ...
        if (!this.train()) {
            if (!this.layLow()) {
                ...
                if (optStat == "both money and respect") {
                    ...
                    tasks.forEach((task, idx) => tasks.sort((a, b) => idx % 2 == 0 ? b.respect - a.respect : b.money - a.money));
                } else {
    ...
    protected needsInitialCombatTraining(): boolean {
        return this.str < this.initStr ||
            this.def < this.initDef ||
            this.dex < this.initDex ||
            this.agi < this.initAgi;
    }
    ...
    public tasks(): ITask[] {
        return this.ns.gang.getTaskNames()
    ...
    protected expectedAscensionResult(): GangMemberAscension {
        return this.mafia.gang.getAscensionResult(this.name) ?? {
    ...
    public ascend() {
        ...
        if (importantStats.some(stat => expectedResults[stat] >= ascMultiThreshold)) {
            if (this.ns.gang.ascendMember(this.name) !== undefined) {
                this.ns.print(`${new Date().toISOString()} Ascended member ${this.name} to increase multis by ${importantStats.map(s => `${s} -> ${expectedResults[s].toFixed(2)}x`).join(", ")}`);
            }
        }
    }
}
