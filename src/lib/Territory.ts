...
export class Territory {
  ...
  protected meansWar(): boolean {
    return (
      (this.isPreparedForNextWarTick() && this.myPowerChanged()) ||
  ...
  public didAWar(): void {
    if (this.myPowerChanged()) {
      this.ns.print(
        `${new Date().toISOString()} ERROR: Territory power updated from ${formatNumberShort(
          this.mostRecentPower
        )} to ${formatNumberShort(this.mafia.info().power)}.`
      );
    }
