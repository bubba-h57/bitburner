...
const factions = [
  ...
  'Tian Di Hui',
  'CyberSec',
]; //TODO: Add Bladeburner Automation at BN7.1
...
export function autocomplete(data, args) {
  ...
  if (lastFlag == '--first' || lastFlag == '--skip') return factions.map((f) => f.replaceAll(' ', '_')).sort();
  return [];
}
...
export async function main(ns) {
  ...
  dictFactionFavors = await getNsDataThroughFile(
    ns,
    dictCommand(factions, 'ns.getFactionFavor(o)'),
  ...
  let installedAugmentations = await getNsDataThroughFile(
    ns,
  ...
  let completedFactions = Object.keys(mostExpensiveAugByFaction).filter(
    (fac) => mostExpensiveAugByFaction[fac] == -1 && !factionSpecificConfigs.find((c) => c.name == fac)?.forceUnlock
  );
  ...
  while (true) {
    ...
    for (const faction of factionWorkOrder) await workForSingleFaction(ns, faction);
    if (scope <= 2) continue;
  ...
  while ((currentReputation = ns.getFactionRep(factionName)) < factionRepRequired) {
    ...
    if (
      ...
      repGainRatePerMs =
        ...
        (hasFocusPenaly && !shouldFocusAtWork
          ? 0.8
    ...
    if (lastFactionWorkStatus != status || Date.now() - lastStatusUpdateTime > statusUpdateInterval) {
      ...
      if (currentReputation > lastRepMeasurement + statusUpdateInterval * repGainRatePerMs * 2)
      ...
      ns.print(
        (lastFactionWorkStatus = status) +
          ...
          (hasFocusPenaly && !shouldFocusAtWork ? 'after 20% non-focus Penalty ' : '') +
...
export async function workForAllMegacorps(
  ...
  if (alsoWorkForCompanyFactions && !oneCompanyFactionAtATime) {
    ...
    for (const factionName of megacorpFactionsInPreferredOrder) await workForSingleFaction(ns, factionName);
  }
}
...
export async function workForMegacorpFactionInvite(ns, factionName, waitForInvite) {
  ...
  if (
    (await getNsDataThroughFile(ns, 'ns.checkFactionInvitations()', '/Temp/player-faction-invites.txt')).includes(
      factionName
    )
  )
    ...
    if (lastStatus != status || Date.now() - lastStatusUpdateTime > statusUpdateInterval) {
      ...
      ns.print(
        `Currently a "${
          ...
          (hasFocusPenaly && !shouldFocusAtWork ? ' and 20% non-focus Penalty' : '') +
          `)\n` +
