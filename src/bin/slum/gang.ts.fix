import { EquipmentStats, GangGenInfo, GangMemberInfo, GangTaskStats, NS, SourceFileLvl } from 'Bitburner';
import {
  formatMoney,
  formatNumberShort,
...
const maxSpendPerTickTransientEquipment: number = 0.002;
const maxSpendPerTickPermanentEquipment: number = 0.2; // Spend up to this percent of non-reserved cash on permanent member upgrades
const wantedPenaltyThreshold: number = 0.0001; // Don't let the wanted penalty get worse than this
...
const territoryEngageThreshold: number = 0.7; // Minimum average win chance (of gangs with territory) before we engage other clans
let territoryTickDetected: boolean = false;
let territoryTickTime: number = 20000; // Est. milliseconds until territory *ticks*. Can vary if processing offline time
let territoryNextTick: number = 0; // The next time territory will tick
let isReadyForNextTerritoryTick = false;
let warfareFinished: boolean = false;
let lastTerritoryPower: number = 0;
...
let pctTraining = 0.2;
let multGangSoftcap: number;
let allTaskNames: string[];
let allTaskStats: GangTaskStats[];
...
// Global state
let ownedSourceFiles: SourceFileLvl[];
let myGangFaction: string = '';
let isHackGang: boolean = false;
let requiredRep: number = 0;
let myGangMembers: string[] = [];
let equipments: equipment[] = [];
let importantStats: string[] = [];

let options: [string, string | number | boolean | string[]][];
...
export async function main(ns: NS) {
  ...
  let thisBitnode = ns.getPlayer().bitNodeN;
  if (!ownedSourceFiles.some((sourceFile: SourceFileLvl) => sourceFile.n === thisBitnode)) {
    ownedSourceFiles.push({
      n: thisBitnode,
      lvl: 3,
    });
  }

  let sf2: SourceFileLvl | undefined = ownedSourceFiles.find((sourceFile: SourceFileLvl) => sourceFile.n === 2);

  const sf2Level = sf2 === undefined ? 0 : sf2.lvl;
  ...
  while (true) {
    try {
      ...
      log(ns, `ERROR: Caught an unhandled error in the main loop: ${String(err)}`, 'error', true);
    }
...
async function initialize(ns: NS) {
  ...
  const sf4Level = ownedSourceFiles[4][1] || 0;
  requiredRep = -1;
  if (sf4Level == 0)
  ...
  const dictMembers: GangMemberInfo[] = await getGangInfoDict(ns, myGangMembers, 'getMemberInformation');
...
async function mainLoop(ns) {
  ...
  if (
    ...
    Date.now() > territoryNextTick + 5000 /* Wait up to 5 additional seconds in case time was wonkey */
  ) {
...
async function onTerritoryTick(ns: NS, myGangInfo) {
  ...
  if (myGangMembers.length < 12 /* Game Max */ && myGangInfo.respect * 0.75 > nextMemberCost)
...
async function updateMemberActivities(
  ns: NS,
  dictMemberInfo: GangMemberInfo | null = null,
  forceTask: string | null = null
) {
  const dictMembers: GangMemberInfo[] =
    dictMemberInfo || (await getGangInfoDict(ns, myGangMembers, 'getMemberInformation'));
  const workOrders: {
    name: string;
    task: string;
  }[] = [];
  ...
  for (const member of Object.values(dictMembers)) {
    ...
    if (member.task != task) workOrders.push({ name: member.name, task: task }); // Only bother with the API call if this isn't their current task
  }
  ...
  if (
    await getNsDataThroughFile(
      ns,
      `${JSON.stringify(workOrders)}.reduce((success, m) => success && ns.gang.setMemberTask(m.name, m.task), true)`,
  ...
  else log(ns, `ERROR: Failed to set member task of one or more members: ` + JSON.stringify(workOrders), 'error');
}

/** @param {NS} ns
 * Logic to assign tasks that maximize rep gain rate without wanted gain getting out of control **/
async function optimizeGangCrime(ns: NS, myGangInfo) {
  const dictMembers = await getGangInfoDict(ns, myGangMembers, 'getMemberInformation');
  ...
  let factionRep = -1;
  if (ownedSourceFiles[4][1] > 0) {
    try {
      factionRep = await getNsDataThroughFile(ns, `ns.getFactionRep('${myGangFaction}')`, `/Temp/gang-faction-rep.txt`);
    } catch {
  ...
  const memberTaskRates: { [k: string]: task[] } = Object.fromEntries(
    Object.values(dictMembers).map((m: GangMemberInfo) => [
      m.name,
      allTaskNames
        .map((taskName) => ({
          name: taskName,
  ...
  for (let shuffle = 0; shuffle < 100; shuffle++) {
    ...
    shuffleArray(myGangMembers.slice()).forEach((member, index) => {
      const taskRates: task[] = memberTaskRates[member];
      ...
      if (
        ...
        bestTask =
          taskRates.find((t) => t.name === 'Train ' + (isHackGang ? 'Hacking' : 'Combat')) ?? sustainableTasks[0];
      } else if (totalWanted > wantedGainTolerance || sustainableTasks.length == 0) {
        bestTask = taskRates.find((t) => t.name === 'Vigilante Justice') ?? sustainableTasks[0];
      } else {
        bestTask = sustainableTasks[0];
      }
    ...
    while (
      ...
      Object.values(proposedTasks).some((t: task) => t.name !== 'Vigilante Justice')
    ) {
  ...
  if (bestTaskAssignments != null && myGangMembers.some((m) => assignedTasks[m] !== bestTaskAssignments[m].name)) {
    ...
    log(
      ...
      `SUCCESS: Optimized gang member crimes for ${optStat} with wanted gain tolerance ${wantedGainTolerance.toPrecision(
        ...
        `Wanted: ${oldGangInfo.wantedLevelGainRate.toPrecision(3)} -> ${myGangInfo.wantedLevelGainRate.toPrecision(
          3
        )}, ` +
        `Rep: ${formatNumberShort(oldGangInfo.respectGainRate)} -> ${formatNumberShort(
          myGangInfo.respectGainRate
        )}, Money: ${formatMoney(oldGangInfo.moneyGainRate)} -> ${formatMoney(myGangInfo.moneyGainRate)}`
    );
    ...
    if (
      ...
      log(
        ns,
        `WARNING: Calculated new rates would be Rep:${formatNumberShort(optRespect)} Wanted: ${optWanted.toPrecision(
          3
        )} Money: ${formatMoney(optMoney)}` +
          `but they are Rep:${formatNumberShort(
            myGangInfo.respectGainRate
          )} Wanted: ${myGangInfo.wantedLevelGainRate.toPrecision(3)} Money: ${formatMoney(myGangInfo.moneyGainRate)}`,
        'warning'
      );
  } else
    log(
...
async function fixWantedGainRate(ns: NS, myGangInfo, wantedGainTolerance = 0) {
  ...
  log(
    ...
    `WARNING: Generating wanted levels (${lastWantedLevelGainRate.toPrecision(
      3
    )}/sec > ${wantedGainTolerance.toPrecision(3)}/sec), temporarily assigning random members to Vigilante Justice...`,
    'warning'
  );
...
async function tryAscendMembers(ns) {
  ...
  for (let i = 0; i < myGangMembers.length; i++) {
    ...
    if (
      undefined !==
      (await getNsDataThroughFile(ns, `ns.gang.ascendMember('${member}')`, '/Temp/gang-ascend-member.txt'))
    ) {
      log(
        ns,
    ...
    } else log(ns, `ERROR: Attempt to ascended member ${member} failed. Go investigate!`, 'error');
  }
}

/** @param {NS} ns
 * Upgrade any missing equipment / augmentations of members if we have the budget for it **/
async function tryUpgradeMembers(ns: NS, dictMembers: GangMemberInfo[]) {
  ...
  if (!playerData.has4SDataTixApi || playerData.bitNodeN === 8) (budget /= 100), (augBudget /= 100);
  if (budget <= 0) return;
  // Find out what outstanding equipment can be bought within our budget
  for (const equip of equipments) {
    dictMembers.forEach((member) => {
      ...
      let percievedCost =
        equip.cost * (Object.keys(equip.stats).some((stat) => importantStats.some((i) => stat.includes(i))) ? 1 : 50);
      if (percievedCost > augBudget) return;
      if (equip.type != 'Augmentation' && percievedCost > budget) return;
...
async function doUpgradePurchases(ns: NS, purchaseOrder) {
  ...
  if (
    ...
    log(
      ns,
      `SUCCESS: Purchased ${purchaseOrder.length} gang member upgrades for ${formatMoney(
        purchaseOrder.reduce((t, e) => t + e.cost, 0)
      )}. (${orderSummary})`,
      'success'
    );
  else log(ns, `ERROR: Failed to purchase one or more gang member upgrades. (${orderSummary})`, 'error');
}

/** @param {NS} ns
 * Helper to wait for the game to update stats (typically 2 seconds per cycle) **/
async function waitForGameUpdate(ns: NS, oldGangInfo: GangGenInfo) {
  ...
  var latestGangInfo: GangGenInfo = ns.gang.getGangInformation();
  while (Date.now() < start + maxWaitTime) {
    latestGangInfo = ns.gang.getGangInformation();
    if (JSON.stringify(latestGangInfo) != JSON.stringify(oldGangInfo)) return latestGangInfo;
...
async function enableOrDisableWarfare(ns: NS, myGangInfo) {
      ...
      (warfareFinished ? 'SUCCESS' : 'INFO') +
        ...
        (!warfareFinished
          ? `Lowest win chance is ${(100 * lowestWinChance).toFixed(
              2
            )}% with ${lowestWinChanceGang} (power ${formatNumberShort(otherGangs[lowestWinChanceGang]?.power)}). ` +
...
const getGangInfoDict = async (ns: NS, elements: string[], gangFunction: string) =>
  await getDict(ns, elements, `gang.${gangFunction}`, `/Temp/gang-${gangFunction}.txt`);

const getDict = async (ns: NS, elements: string[], nsFunction: string, fileName: string) =>
  await getNsDataThroughFile(
    ns,
    `Object.fromEntries(${JSON.stringify(elements)}.map(e => [e, ns.${nsFunction}(e)]))`,
    fileName
  );
...
/** @param {NS} ns **/
function log(ns, message: string, toastStyle?: string, terminal?: boolean) {
  ns.print(message);
